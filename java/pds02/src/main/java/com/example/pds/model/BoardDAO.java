...
public class BoardDAO {
	...
	public int boardWriteOk( BoardTO to ) {
		...
		try {
			...
			String sql = "insert into pds_adv_board1 values ( 0, ?, ?, ?, ?, md5( ? ), ?, ?, ?, 0, ?, now() )";
			...
			pstmt.setString( 7, to.getFileName() );
			pstmt.setInt( 8, to.getFileSize() );
	...
	public ArrayList<BoardTO> boardList() {
		...
		try {
			...
			String sql = "select seq, kind, subject, writer, filename, wdate, hit, datediff( now(), wdate ) wgap from pds_adv_board1 order by seq desc";
	...
	public BoardTO boardView( BoardTO to ) {
		...
		try {
			...
			String sql = "update pds_adv_board1 set hit=hit+1 where seq=?";
			pstmt = conn.prepareStatement( sql );
			pstmt.setString( 1, to.getSeq() );

			pstmt.executeUpdate();

			sql = "select subject, kind, writer, mail, wip, wdate, hit, content, filename, filesize from pds_adv_board1 where seq=?";
			pstmt = conn.prepareStatement( sql );
			pstmt.setString( 1, to.getSeq() );
	...
	public BoardTO boardDelete( BoardTO to ) {
		...
		try {
			...
			String sql = "select subject, writer from pds_adv_board1 where seq=?";
			pstmt = conn.prepareStatement( sql );
			pstmt.setString( 1, to.getSeq() );
	...
	public int boardDeleteOk( BoardTO to ) {
		...
		try {
			...
			String sql = "select filename from pds_adv_board1 where seq=?";
			pstmt = conn.prepareStatement( sql );
			pstmt.setString( 1, to.getSeq() );
			...
			sql = "delete from pds_adv_board1 where seq=? and password = md5( ? )";
			pstmt = conn.prepareStatement( sql );
			pstmt.setString( 1, to.getSeq() );
			...
			if ( result == 0 ) {
				...
				if ( !fileName.equals( "" ) ) {
					new File("c:/git/Back_End_Study/java/upload", fileName).delete();
				}
			}
		} catch( SQLException e ) {
			System.out.println( "[에러] " + e.getMessage() );
	...
	public BoardTO boardModify( BoardTO to ) {
		...
		try {
			...
			String sql = "select kind, subject, writer, mail, content, filename from pds_adv_board1 where seq = ?";
			pstmt = conn.prepareStatement( sql );
			pstmt.setString( 1, to.getSeq() );
